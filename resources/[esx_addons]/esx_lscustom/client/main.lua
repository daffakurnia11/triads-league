...
RegisterNetEvent('custom_repair:doRepair')
AddEventHandler('custom_repair:doRepair', function()
    ...
    if vehicle ~= 0 then
        ...
        local success = lib.progressCircle({
            duration = 8000,
            label = 'Sedang memperbaiki kendaraan...',
            position = 'bottom',
            useWhileDead = false,
            canCancel = false,
            disable = {
        ...
        if success then
            ...
            ESX.ShowNotification("✅ Kendaraanmu berhasil diperbaiki!")
        end
    else
        ESX.ShowNotification("⚠️ Kamu harus berada di dalam kendaraan!")
    end
end)

RegisterNetEvent('custom_repair:insufficientFunds')
AddEventHandler('custom_repair:insufficientFunds', function()
    ESX.ShowNotification("❌ Kamu tidak memiliki cukup uang untuk memperbaiki kendaraan!")
end)
...
RegisterNetEvent('item_repair:useKit')
AddEventHandler('item_repair:useKit', function(args)
    local ped = PlayerPedId()
    local vehicle = GetVehiclePedIsIn(ped, false)
    local kitType = args
    ...
    if vehicle ~= 0 and GetPedInVehicleSeat(vehicle, -1) == ped then
        ...
        local label = 'Memperbaiki kendaraan...'
        ...
        if kitType == 'toolkit' then
            duration = 15000
            label = 'Menggunakan Toolkit (perbaikan total)...'
        elseif kitType == 'repairkit' then
            duration = 10000
            label = 'Memperbaiki mesin...'
        elseif kitType == 'bodykit' then
            duration = 10000
            label = 'Memperbaiki body/chassis...'
        elseif kitType == 'tirekit' then
            duration = 5000
            label = 'Memperbaiki ban...'
        end
        ...
        if not ok then
            ESX.ShowNotification('❌ Perbaikan Dibatalkan.')
            return false
        end
        ...
        TriggerServerEvent("item_repair:consumeKit", kitType)
    else
        ESX.ShowNotification("⚠️ Kamu harus berada di dalam kendaraan!")
    end
end)

RegisterNetEvent('item_repair:useToolKit')
AddEventHandler('item_repair:useToolKit', function()
    TriggerEvent('item_repair:useKit', 'toolkit')
end)

RegisterNetEvent('item_repair:useRepairKit')
AddEventHandler('item_repair:useRepairKit', function()
    TriggerEvent('item_repair:useKit', 'repairkit')
end)

RegisterNetEvent('item_repair:useBodyKit')
AddEventHandler('item_repair:useBodyKit', function()
    TriggerEvent('item_repair:useKit', 'bodykit')
end)

RegisterNetEvent('item_repair:useTireKit')
AddEventHandler('item_repair:useTireKit', function()
    TriggerEvent('item_repair:useKit', 'tirekit')
end)
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), menuName, {
        ...
        for k, v in pairs(Config.Menus) do
            ...
            if k == data.current.modType or isRimMod then
                ...
                if data.current.label == TranslateCap('by_default') or
                    string.match(data.current.label, TranslateCap('installed')) then
            ...
            if v.modType then
                ...
                if v.modType == 14 then -- HORNS
                    for j = 0, 51, 1 do
                        ...
                        if j == currentMods.modHorns then
                            _label = GetHornName(j) .. ' - <span style="color:cornflowerblue;">' ..
                ...
                elseif v.modType == 22 then -- NEON
                    ...
                    if currentMods.modXenon then
                        _label = TranslateCap('neon') .. ' - <span style="color:cornflowerblue;">' ..
                                     TranslateCap('installed') .. '</span>'
                    else
                        price = math.floor(vehiclePrice * v.price / 100)
                ...
                elseif v.modType == 'windowTint' then -- WINDOWS TINT
                    for j = 1, 5, 1 do
                        ...
                        if j == currentMods.windowTint then
                            _label = GetWindowName(j) .. ' - <span style="color:cornflowerblue;">' ..
                                         TranslateCap('installed') .. '</span>'
                        else
                            price = math.floor(vehiclePrice * v.price / 100)
                ...
                elseif v.modType == 11 or v.modType == 12 or v.modType == 13 or v.modType == 15 or v.modType == 16 then
                    ...
                    for j = 0, modCount, 1 do
                        ...
                        if j == currentMods[k] then
                            _label = TranslateCap('level', j + 1) .. ' - <span style="color:cornflowerblue;">' ..
                                         TranslateCap('installed') .. '</span>'
                        else
                            price = math.floor(vehiclePrice * v.price[j + 1] / 100)
                            _label = TranslateCap('level', j + 1) .. ' - <span style="color:green;">$' .. price ..
                                         ' </span>'
                        end
                        elements[#elements + 1] = {
                            label = _label,
                ...
                elseif v.modType == 17 then -- TURBO
                    ...
                    if currentMods[k] then
                        _label = 'Turbo - <span style="color:cornflowerblue;">' .. TranslateCap('installed') ..
                                     '</span>'
                    else
                        _label =
...
CreateThread(function()
    while true do
        ...
        if IsPedInAnyVehicle(playerPed, false) then
            ...
            for k, v in pairs(Config.Zones) do
                if #(coords - v.Pos) < v.Distance then
                    Near = true
                    Sleep = 0
                        ...
                        if v.Action == 'custom' then
                            ...
                            if IsControlJustReleased(0, 38) then
                                ...
                                if vehicle ~= 0 and GetPedInVehicleSeat(vehicle, -1) == playerPed then
                                    ...
                                    ESX.ShowNotification("⚠️ Harus berada di kursi sopir.")
                                end
                            end
                        end
                    end
